...
def get_messages_history(history_len: int, isDetailed=False) -> List[Dict]:
    def filter(msg):
        '''
        闁藉牆顕ぐ鎾冲缁犫偓閸楁洘鏋冮張顒€顕拠婵撶礉閸欘亣绻戦崶鐐寸槨閺夆剝绉烽幁顖滄畱缁楊兛绔存稉鐚爈ement閻ㄥ嫬鍞寸€癸拷
        '''
...
def dialogue_page(api: ApiRequest):
    ...
    with st.sidebar:
        ...
        if dialogue_mode == webui_configs["dialogue"]["mode"][1]:
            ...
            tool_selects = []
            if tool_using_on:
                with st.expander("瀹搞儱鍙块崘娑氫紑鎼达拷", True):
    ...
    if prompt := st.chat_input(chat_input_placeholder, key="prompt"):
        ...
        if dialogue_mode == webui_configs["dialogue"]["mode"][0]:
            ...
            chat_box.update_msg(text, streaming=False)  # 閺囧瓨鏌婇張鈧紒鍫㈡畱鐎涙顑佹稉璇х礉閸樺娅庨崗澶嬬垼
            # 閸掋倖鏌囬弰顖氭儊鐎涙ê婀禒锝囩垳, 楠炶埖褰佹妯肩椽鏉堟垵濮涢懗鏂ょ礉閹笛嗩攽閸旂喕鍏�
            ...
            chat_box.update_msg(text, element_index=0, streaming=False, state="complete")  # 閺囧瓨鏌婇張鈧紒鍫㈡畱鐎涙顑佹稉璇х礉閸樺娅庨崗澶嬬垼
            if search_on:
            ...
            for idx_count, d in enumerate(
                ...
                # chat_box.update_msg("閻儴鐦戞惔鎾冲爱闁板秶绮ㄩ弸锟�: \n\n".join(d["docs"]), element_index=1, streaming=False, state="complete")
            chat_box.update_msg(text, element_index=0, streaming=False)  # 閺囧瓨鏌婇張鈧紒鍫㈡畱鐎涙顑佹稉璇х礉閸樺娅庨崗澶嬬垼
